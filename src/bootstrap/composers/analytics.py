...
def _analytics_repository() -> tuple[UnitOfWork, SqlAlchemyAnalyticsQueryRepository]:
    unit_of_work = UnitOfWork(DATABASE_ENGINE)
    return unit_of_work, SqlAlchemyAnalyticsQueryRepository(unit_of_work)


def _dashboard_repository() -> tuple[UnitOfWork, DashboardWorkspaceQueryRepository]:
    unit_of_work = UnitOfWork(DATABASE_ENGINE)
...
def get_dashboard_overview_composer() -> TransactionalHandler:
    unit_of_work, repository = _dashboard_repository()
...
def list_customer_metrics_composer() -> TransactionalHandler:
    unit_of_work, repository = _analytics_repository()
