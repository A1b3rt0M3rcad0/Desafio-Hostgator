...
export const api = {
  ...
  listTickets: (query, signal) => request('/tickets', { query, signal }),
  getTicket: (id) => request(`/tickets/${id}`),
  listCustomers: (query, signal) => request('/customers', { query, signal }),
  ...
  getIngestionControl: () => request('/ingestion/control'),
  updateIngestionControl: (enabled) => request('/ingestion/control', { method: 'PATCH', body: { enabled } }),
  listCustomerMetrics: (query, signal) => request('/metrics/customers', { query, signal }),
  getExportCatalog: () => request('/exports/catalog'),
  previewDataExport: (body) => request('/exports/data/preview', { method: 'POST', body }),
  exportData: (body) => download('/exports/data/download', body, `tickets-detalhados.${body.format || 'csv'}`),
  exportMetrics: (body) => download('/exports/metrics/download', body, `metricas.${body.format || 'csv'}`),
  exportMetricsReport: (body) => download('/exports/metrics/download', body, `metricas.${body.format || 'csv'}`),
};
